...
def get_top_variables_for_topic(topic: str, top_n: int = 5, preselect_n: int = 75):
    ...
    prompt = f"""
    Bir veri analistisiniz. "{topic}" konusunu analiz etmek için aşağıdaki veri kümesi değişkenlerinden en alakalı {top_n} tanesini seçin.
    Veri Kümesi Değişkenleri:
    {reduced_metadata}
    Sadece değişken adlarını virgülle ayırarak yazın (örneğin: age, income, education_level).
    Açıklama eklemeyin.
    """
    ...
    response = client.chat.completions.create(
        ...
        messages=[
            {"role": "system", "content": "Tüm cevaplarını Türkçe olarak ver."},
...
def suggest_questions(topic: str):
    ...
    prompt_suggest = f"""
    Bir veri analistisiniz. Aşağıda verilen veri değişkenleri ve kodlamalara göre, 'data_2010' veri setini daha iyi anlamaya yardımcı olacak 10 çeşitli ve anlamlı soru önerin.
    Bu sorular SQL sorguları ile cevaplanabilir olmalıdır. Sorular Turkçe olarak yazılmalıdır.

    Veri Değişkenleri:
    {metadata_text}
    {mapping_section}
    Sadece numaralı bir soru listesi verin. Açıklama eklemeyin.
    """
    ...
    response = client.chat.completions.create(
        ...
        messages=[
            {"role": "system", "content": "Tüm cevaplarını Türkçe olarak ver."},
            {"role": "user", "content": prompt_suggest}
        ],
        temperature=0.4
    )
...
def generate_sql(question: str):
    ...
    prompt_sql = f"""
    Bir veri analisti olarak 'data_2010' isimli veri seti üzerinde çalışıyorsunuz. Kullanıcının aşağıdaki sorusunu, verilen veri sözlüğüne göre cevaplayacak doğru bir **SQLite SQL sorgusu** yazın.
    Sorgu, kullanıcının amacını doğru şekilde yansıtmalı ve en alakalı sütunları kullanmalıdır.

    Tüm sütun adlarını çift tırnak içinde yazın.
    ...
    **Sadece SQL sorgusunu döndür, baska açıklama yazma.**
    """
...
def summarize_answer(question: str, result_df: pd.DataFrame):
    ...
    prompt_answer = f"""
    Yardımcı bir veri analisti asistansınız. Aşağıdaki özet tabloya dayanarak, kullanıcı sorusunu yanıtlayan kısa ve açıklayıcı bir yorum yapın.
    Verideki önemli eğilimleri, dikkat çeken noktaları veya anomalileri vurgulayın. Veriyi tekrar etmeyin; anlamını açıklayın.
    Açıklamanız teknik olmayan bir izleyici kitlesi için anlaşılır olmalı.
